...
use image::{open, ImageFormat, Rgb, RgbImage};

use crate::image_processing::color_mappers::{LabPaletteMapper, RgbPaletteMapper};
use crate::image_processing::palette_extraction::{
    create_lab_palette_mapper, create_rgb_palette_mapper, create_swap_palette_mapper,
};
use crate::image_processing::utils::{
    load_image_from_file, load_image_from_unknown_reader, save_image,
};
...
fn test_dither_with_palette() {
    ...
    let donor_image = load_image_from_file(DONOR_PIC).expect("Cannot load image");
    let mut img_to_process = load_image_from_file(RECEPTOR_PIC).expect("Cannot load image");

    let color_mapper = create_rgb_palette_mapper(donor_image, PALETTE_QUANTITY);

    let mut img_to_process = img_to_process.to_rgb8();
    img_to_process.dither_with_palette(color_mapper);
    img_to_process
        .save_with_format("gen_test_dither", ImageFormat::Png)
...
fn test_apply_palette_to_image() {
    ...
    let donor_image = load_image_from_file(DONOR_PIC).expect("Cannot load image");
    let img_to_process = load_image_from_file(RECEPTOR_PIC).expect("Cannot load image");
    ...
    img_to_process
        .save_with_format("gen_test_palette", ImageFormat::Png)
        .expect("Failed to save file");
...
fn test_swap_palette() {
    ...
    let color_mapper = create_swap_palette_mapper(&donor_image, &img_to_process, PALETTE_QUANTITY);

    let mut img_to_process = img_to_process.to_rgb8();
    img_to_process.apply_palette_to_image(color_mapper);
    img_to_process
        .save_with_format("gen_test_swap", ImageFormat::Png)
        .expect("Failed to save file");
